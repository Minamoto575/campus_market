...
<div class="top clearfix" id ="user-top">
    <div id="user_msg">
        ...
        <ul class="seller_attr">
            <li>已发布书籍：<span id="goodsTotal">45</span></li>
            <li>被收购书籍：<span id = "hasAcquired">4545</span></li>
            <li>未收购书籍：<span id = "notAcquired">4545</span></li>
            <li>待求购书籍：<span id = "userpoint">${wantedGoodsList?size}</span></li>
            <li>已建议书籍：<span id = "userpoint">${reportGoodsList?size}</span></li>
            <li>已购买书籍：<span id = "userpoint">${orderFormList?size}</span> </li>
            <li>查看：<span><a style="color: #00AC90" href="/home/student/order">书籍订单</a></span>
            </li>
        </ul>
    </div>
    <div id="user_photo">
...
<div class="pop-tip hide">
    <div class="pop-tip-area">
        ...
        <ul id="middle_nav" class="clearfix">
   		<li class="on"><a href="/home/student/publish">我发布的出售书籍</a></li>
		</ul>
        <div id="my_products">
              <div id="onsale_pro">
                <#if studentGoodsList??>
                 <#list studentGoodsList as studentGoods>
                 <div class="enshr_each" id="">
	                    <div class="enshr_info">
	                        <h2><a href="../student_goods/detail?id=${studentGoods.id}" title="${studentGoods.name}">${studentGoods.name}</a></h2>
	                        <p style="overflow:hidden;">${studentGoods.content}</p>
	                        <div class="enshr_state">
	                        <span id="prostate">
	                        	状态：
	                        	<#if studentGoods.status ==1>
	                        	<font style="color:rgb(75, 192, 165);">未被收购</font>
	                        	<#else>
	                        	<font style="color:#4BC00F;">已被收购</font>
	                        	</#if>
	                        </span>|
	                        <span id="prostate">上架日期：${studentGoods.createTime}</span>
	                           	<#if studentGoods.status == 1>
                                    <span class="enshrine_it make_edition" style="margin-right:30px;" onclick="del(${studentGoods.id});">删除</span>
                                    <a href="/home/student_goods/edit_goods?id=${studentGoods.id}" target="_top">
                                        <span class="enshrine_it make_edition">编辑</span>
                                    </a>
	                            <#elseif studentGoods.status == 2>
                                    <a href="../student_goods/detail?id=${studentGoods.id}">
                                        <span class="enshrine_it make_edition" style="margin-right:30px;">查看</span>
                                    </a>
                                </#if>
	                        </div>
	                    </div>
	                    <a href="../student_goods/detail?id=${studentGoods.id}">
	                        <img class="enshr_ph" src="/photo/view?filename=${studentGoods.photo}" alt="${studentGoods.name}">
	                    </a>
                </div>
                </#list>
                </#if>
        ...
        <ul id="middle_nav" class="clearfix">
   		<li class="on"><a href="/home/student/publish_wanted">我发布的求购书籍</a></li>
		</ul>
        <div id="my_products">
              <div id="onsale_pro">
                <#if wantedGoodsList??>
        ...
        <ul id="middle_nav" class="clearfix">
   		<li class="on"><a href="">我建议的书籍</a></li>
		</ul>
        <div id="my_products">
              <div id="onsale_pro">
                <#if reportGoodsList??>
                 ...
                 <div class="enshr_each" id="">  
	                    <div class="enshr_info">
	                        ...
	                        <div class="enshr_state">
	                        &nbsp;&nbsp;<span id="prostate">建议日期：${reportGoods.createTime}</span>
	                        &nbsp;&nbsp;<span id="prostate">建议原因：${reportGoods.content}</span>
...
$(document).ready(function(){
	ajaxRequest('get_stats','post',{},function(rst){
		...
		$("#notAcquired").text(rst.data.notAcquired);
		$("#hasAcquired").text(rst.data.hasAcquired);
	});
