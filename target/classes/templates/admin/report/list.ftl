...
<title>${siteName!""}|图书建议管理-${title!""}</title>
                  ...
                  <div class="input-group">
                    <div class="input-group-btn">
                      <button class="btn btn-default dropdown-toggle" id="search-btn" data-toggle="dropdown" type="button" aria-haspopup="true" aria-expanded="false">
                      	<#if name??>图书名称<#elseif sn??>学生学号<#elseif content??>建议原因<#else>搜索条件</#if> <span class="caret"></span>
                      </button>
                      <ul class="dropdown-menu">
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="content">建议原因</a> </li>
...
function deleteReq(id,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('图书建议删除成功!',function(){
